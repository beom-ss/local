...
/*예제 : computer repair*/
data repair;
	infile"C:\Users\kuksunghee\Desktop\repair.txt";
	input minutes units;
run;
...
proc sql;
	...
	select minutes as y, units as x, 
              avg(x) as xbar, avg(y) as ybar, y-avg(y) as y_ybar, x-avg(x) as x_xbar,
              (y-avg(y))**2 as y_ybar2, (x-avg(x))**2 as x_xbar2, (y-avg(y))*(x-avg(x)) as xy
	from repair;
quit;
proc sql;
...
data news;
	infile"C:\Users\kuksunghee\Desktop\newspapers.txt" dlm="," ;
	length newspaper$ 30 ;
